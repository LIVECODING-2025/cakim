...
$mingguIniQuery = "SELECT * FROM event 
                    WHERE status = 'disetujui' 
                    AND tanggal BETWEEN '$today' AND '$nextWeek'
                    ORDER BY tanggal ASC";
$mingguIniResult = $conn->query($mingguIniQuery);
?>
...
<head>
  ...
  <style>
    ...
    .card-custom {
      border-radius: 15px; background: #f0f6ff; border: none;
    ...
    .alert-custom {
      background: #d1e7ff; color: #084298;
      font-weight: 500; border-radius: 8px; padding: 15px;
    }
    .badge {
      position: absolute; top: 10px; left: 10px;
      background: #007aff; color: white;
      padding: 5px 10px; font-size: 14px;
      border-radius: 15px; animation: pulse 2s infinite;
    }
    @keyframes pulse {
      0% { transform: scale(1); }
    ...
    <div class="col-md-9">
      <div class="main-content">
        ...
        <div class="section-title mt-5">
          <h2>Event Minggu Ini</h2>
        </div>
        <div class="row g-4 mb-4">
          <?php if ($mingguIniResult->num_rows > 0): ?>
            <?php while ($event = $mingguIniResult->fetch_assoc()): ?>
              <div class="col-md-4">
                <div class="card-custom text-center p-3 position-relative">
                  ...
                  <h5 class="mt-3"><?= htmlspecialchars($event['nama_event']) ?></h5>
                  <p class="text-muted"><?= ucfirst($event['kategori']) ?> - <?= date("d M Y", strtotime($event['tanggal'])) ?></p>
                  <a href="detail.php?id=<?= $event['id_event'] ?>" class="btn btn-primary btn-sm mt-2">Lihat Detail</a>
                </div>
              </div>
            <?php endwhile; ?>
          <?php else: ?>
            <p class="text-muted">Tidak ada event dalam minggu ini.</p>
          <?php endif; ?>
        </div>
          ...
          <?php while ($row = $result->fetch_assoc()): ?>
            <div class="col-md-4">
              <div class="card-custom text-center p-3 position-relative">
                ...
                <img src="admin/poster/<?= htmlspecialchars($row['poster'] ?? 'default.jpg') ?>" alt="<?= htmlspecialchars($row['nama_event']) ?>">
                ...
                <a href="detail.php?id=<?= $row['id_event'] ?>" class="btn btn-primary btn-sm mt-2">Lihat Detail</a>
              </div>
            </div>
          <?php endwhile; ?>
        </div>
